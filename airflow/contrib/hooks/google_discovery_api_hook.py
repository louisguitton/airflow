...
class GoogleDiscoveryApiHook(GoogleCloudBaseHook):
    ...
    def __init__(
        self, api_service_name, api_version, gcp_conn_id='google_cloud_default', delegate_to=None
    ):
    ...
    def get_conn(self):
        ...
        if not self._conn:
            ...
            self._conn = build(serviceName=self.api_service_name, version=self.api_version,
    ...
    def query(self, endpoint, data, paginate=False):
        ...
        api_response = self._call_api_request(
            google_api_conn_client, endpoint, data, paginate
        )
        return api_response

    def _call_api_request(
        self, google_api_conn_client, endpoint, data, paginate
    ):
    ...
    def _paginate_api(
        ...
        while google_api_endpoint_instance:
            ...
            google_api_endpoint_instance = self._build_next_api_request(
                google_api_conn_client, api_endpoint_parts[1:],
                google_api_endpoint_instance, api_response
            )

        return api_responses

    def _build_next_api_request(
        self, google_api_conn_client, api_sub_functions, api_endpoint_instance,
        api_response
    ):
        for sub_function in api_sub_functions:
            if sub_function != api_sub_functions[-1]:
